...
@EITHON.on(admin_cmd(pattern=f"{WELCOME_CMD}(?:\s|$)([\s\S]*)"))
...
@EITHON.on(admin_cmd(pattern=f"{DELWELCOME_CMD}$"))
...
@EITHON.on(admin_cmd(pattern=f"{WELCOMES_CMD}$"))
...
@EITHON.on(admin_cmd(pattern="الترحيب السابق (تشغيل|ايقاف)$"))
async def del_welcome(event):
