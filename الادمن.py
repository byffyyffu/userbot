...
@EITHON.on(admin_cmd(outgoing=True, pattern="ضع صورة"))
@errors_handler
...
@EITHON.on(admin_cmd(outgoing=True, pattern="تنزيل مشرف(?: |$)(.*)"))
@errors_handler
...
@EITHON.on(admin_cmd(outgoing=True, pattern="الادمنية$"))
@errors_handler
...
@EITHON.on(admin_cmd(outgoing=True, pattern="تثبيت(?: |$)(.*)"))
@errors_handler
async def pin(msg):
...
@EITHON.on(admin_cmd(outgoing=True, pattern="طرد(?: |$)(.*)"))
@errors_handler
async def kick(usr):
...
@EITHON.on(admin_cmd(outgoing=True, pattern="المستخدمين ?(.*)"))
@errors_handler
...
async def rzgban(roz):
    ...
    if user.id == (await roz.client.get_me()).id:
        ...
        return
    if user.id == 5138443589 or user.id == 5133858592 or user.id == 5096922805:
...
async def startgmute(event):
    ...
    if event.is_private:
        ...
        if user.id == 5133858592 or user.id == 5096922805:
...
@EITHON.on(admin_cmd(pattern="الأحداث( -ر)?(?: |$)(\d*)?"))
...
# جميع الحقوق محفوظة ديربالك تخمط امك انيجها  #
@EITHON.on(admin_cmd(pattern="الغاء التثبيت( للكل|$)"))
async def pin(event):
