...
@EITHON.on(admin_cmd(pattern="تقليد$"))
async def echo(event):
    "To echo the user messages"
    if event.reply_to_msg_id is None:
        return await edit_or_reply(event, "⌔∮ يرجى الرد على الشخص الذي تـريد ازعاجه ،")
    EITHONevent = await edit_or_reply(event, "⌔∮ يتم تفعيل هذا الامر انتظر قليلا ،")
    user, rank = await get_user_from_event(event, EITHONevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(EITHONevent, f"⌔∮ خطأ\n`{str(e)}`")
    else:
        await edit_or_reply(
            EITHONevent,
...
@EITHON.on(admin_cmd(pattern="مسح المقلدهم"))
async def echo(event):
...
@EITHON.on(admin_cmd(pattern="الغاء التقليد( -a)?"))
async def echo(event):
...
@EITHON.on(admin_cmd(pattern="المقلدهم( -a)?$"))
...
@EITHON.ar_cmd(incoming=True, edited=False)
