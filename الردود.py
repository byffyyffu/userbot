...
async def filter_incoming_handler(event):  # sourcery no-metrics
    ...
    rozrtba = (
        ".「  مطـور السورس  」."
        if userid == 5133858592 or userid == 5138443589 or userid == 5096922805 or userid == 
        else (".「  العضـو  」.")
    )
    rozrtba = (
        ".「 مـالك الحساب  」."
        ...
        and userid != 5096922805
        and userid != 5138443589
        and userid != 
        else rozrtba
    )
...
@EITHON.on(admin_cmd(pattern="اضف رد (.*)"))
...
@EITHON.on(admin_cmd(pattern="الردود$"))
@EITHON.on(sudo_cmd(pattern="الردود$", allow_sudo=True))
...
@EITHON.on(admin_cmd(pattern="حذف رد ([\s\S]*)"))
...
@EITHON.on(admin_cmd(pattern="حذف الردود$"))
